...
The other two cases, for contrast -- an all-float table, and one that encodes
nothing but still has every column recast:

    uv run scripts/profile_clean_data.py --mix numeric
    uv run scripts/profile_clean_data.py --mix numeric-object
"""
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=[MIX_HALF_STRING, MIX_NUMERIC, MIX_NUMERIC_OBJECT],
        help="Column make-up, generated by the benchmark's own generators, which "
        "also decide the default shape. The mixed one is the case with RAM left to "
        "explain, so it is the default here.",
    )
